...
<div class="container">
  ...
  <div class="card">
    <div class="card-body">
      <form action="ubah-sukses.php" method="post">
      <h1><i class="fas fa-book-open"></i> Ubah buku</h1>
      <br>
      <?php
      require '../../include/koneksi.php';
      $row = mysqli_fetch_assoc(mysqli_query($connect,"SELECT * FROM buku WHERE id = '$_GET[id]'"));
      ?>
      <div class="form-group">
        <label>Nama Buku</label>
        <input type="text" name="nama" class="form-control" placeholder="Masukan nama buku" value="<?= $row['nama']?>"required>
      </div>
      <div class="form-group">
        <label>Jenis Buku</label>
        <input type="text" name="jenis" class="form-control" value="<?= $row['jenis']?>" placeholder="Masukan jenis buku" required>
      </div>
      <div class="form-group">
        <label>Penerbit</label>
        <input type="text" name="penerbit" class="form-control" value="<?= $row['penerbit']?>" placeholder="Masukan nama penerbit" required>
